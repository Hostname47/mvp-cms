...
use App\Models\{User,Visit,ContactMessage,Faq,AuthorRequest,Post,Comment};
use Carbon\Carbon;
...
class AdminController extends Controller
{
    public function dashboard() {
        ...
        $messages = ContactMessage::with(['user'])->where('read', 0)->latest()->paginate(6);
        $faqs = Faq::with(['user'])->where('live', 0)->latest()->paginate(6);
        $author_requests = AuthorRequest::with(['user'])->where('status', '<>', 1)->latest()->paginate(4);
        $posts = Post::where('status', 'published')->orderBy('published_at', 'desc')->take(6)->get();
        $comments = Comment::with(['user','post','post.categories'])->latest()->take(6)->get();

        return view('admin.dashboard')
            ->with(compact('messages'))
            ->with(compact('faqs'))
            ->with(compact('author_requests'))
            ->with(compact('posts'))
