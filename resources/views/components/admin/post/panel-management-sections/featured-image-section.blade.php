<div class="post-management-panel-section toggle-box">
    @php
        $has_featured_image = ($post && $post->has_featured_image()) ? true : false;
    @endphp
    <input type="hidden" id="post-featured-image-metadata-id" autocomplete="off" @if($has_featured_image) value="{{ $post->metadata['featured_image'] }}" @endif>
    ...
    <div class="post-management-panel-section-content toggle-container none">
        <div class="featured-image-box">
            <div class="featured-image-upload-box open-featured-image-selection-viewer @if($has_featured_image) none @endif">
                <span class="bold dark">Set featured image</span>
            </div>
            <div class="uploaded-featured-image-box @if(!$has_featured_image) none @endif">
                <div class="selected-featured-image-container full-center overflow-hidden">
                    <img src="@if($has_featured_image){{ $post->featured_image }}@endif" class="selected-featured-image open-image-on-image-viewer fill-and-center-image-on-parent pointer" alt="">
                </div>
