<div class="flex align-center space-between mb8" style="margin-top: 12px; padding: 0 20px;">
    ...
    <div class="flex-column align-center">
        <img src="{{ $user->avatar(100) }}" class="size60 rounded" style="border: 1px solid #ddd" alt="">
...
<div class="flex align-center mb8">
    ...
    <span class="dark bold">Role informations</span>
</div>

<div class="flex mb8">
    <p class="no-margin dark fs12 bold mr8">Role :</p>
    <div style="margin-top: -1px">
        <h4 class="no-margin fs16 bold lblack">{{ $role->title }}</h4>
        <span class="fs13 gray block" style="margin-top: -1px">{{ $role->slug }}</span>
    </div>
</div>

<p class="lblack no-margin fs13"><span class="bold dark fs12 mr4">Description :</span>{{ $role->description }}</p>

<div style="margin: 12px 0">
    <span class="dark bold fs12 block">Role permissions</span>
    <p class="fs12 light-gray my2">Once the role get revoked from this user, he will lose all the following permissions as well.</p>
    <div class="typical-section-style border-box y-auto-overflow" style="margin: 2px 0 12px 0; max-height: 250px;">
        ...
        <div class="flex flex-wrap" style="gap: 6px;">
            @foreach($role->permissions->groupBy('scope') as $scope=>$permissions)
                <span class="flex bold blue fs11" style="flex-basis: 100%">{{ ucfirst($scope) }}</span>
...
<label for="update-role-confirm-input" class="block fs15 bold dark">Confirmation</label>
<p class="no-margin mb4 dark">Please type <strong>{{ auth()->user()->username }}::revoke-role::{{ $role->slug }}</strong> to confirm.</p>
<div>
    <input type="text" autocomplete="off" class="full-width styled-input" id="revoke-role-confirm-input" style="padding: 8px 10px" placeholder="confirmation">
    <input type="hidden" id="revoke-role-confirm-value" autocomplete="off" value="{{ auth()->user()->username }}::revoke-role::{{ $role->slug }}">
</div>
<div class="flex" style="margin-top: 12px">
    <div id="revoke-role-button" class="typical-button-style red-bs red-bs-disabled full-center">
